...
Le depot contient une fondation Django, un catalogue public de concerts, des fiches detaillees, le modele utilisateur email, les pages d'inscription/connexion/deconnexion/espace personnel, le noyau domaine de billetterie, des services testables, Ruff, coverage, GitHub Actions et une configuration SonarCloud.
...
- `EF10`, `RG6` et `RG8` restent non couverts par absence de vues et de controle d'acces utilisateur ;
...
| EM4 | Concert passe refuse par le domaine, masque du catalogue et explique sur sa fiche | `test_past_concert_not_bookable`, `tests/test_concert_views.py::test_past_concert_detail_explains_unavailability_without_cta` | Couvert domaine et interface | Transition automatique vers le statut termine non implementee |
| EM5 | Concert annule refuse par le domaine, masque du catalogue et explique sur sa fiche | `test_cancelled_concert_not_bookable`, `tests/test_concert_views.py::test_cancelled_concert_detail_explains_unavailability_without_cta` | Couvert domaine et interface | Action d'annulation admin non testee |
...
| ID | Implementation actuelle | Tests actuels | Statut | Limite connue |
| --- | --- | --- | --- | --- |
| ENF1 | Acces direct accueil → catalogue → fiche, CTA de connexion avec retour vers la fiche | `tests/test_homepage.py`, `tests/test_concert_views.py::test_bookable_concert_prompts_anonymous_visitor_to_log_in` | Partiel | Panier et paiement ne sont pas encore exposes |
...
| ID | Implementation actuelle | Tests actuels | Statut | Limite connue |
| --- | --- | --- | --- | --- |
| RG1 | `Concert.is_bookable`, validation service, filtrage de `ConcertListView` et motifs de `ConcertDetailView` | Tests domaine existants et `tests/test_concert_views.py` pour liste, concert passe, sans stock et vente fermee | Couvert domaine et interface | Reservation effective toujours limitee au service panier |

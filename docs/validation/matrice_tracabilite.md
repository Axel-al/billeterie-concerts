...
| EF7 | `payments.forms.SimulatedPaymentForm`, `payments.views.SimulatedPaymentView`, `payments.services.process_simulated_card_payment` | `tests/test_booking_flow.py::test_accepted_payment_creates_paid_order_and_decrements_stock`, `test_refused_payment_does_not_create_validated_order_or_decrement_stock` | Couvert | Paiement simulé deterministe, sans prestataire réel |
...
| ENF4 | Services et formulaires rejettent les saisies invalides proprement ; une incohérence de stock tardive provoque un rollback explicite | Tests quantité/stock/statut domaine, `tests/test_core_domain.py::test_failed_conditional_stock_update_rolls_back_payment`, tests authentification, `tests/test_booking_flow.py` | Couvert pour le périmètre implémenté |  |
...
- limité connue si la couverture est partielle.
